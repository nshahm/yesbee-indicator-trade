...
<div class="space-y-6">
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div class="flex items-center gap-4 flex-1">
      <div class="relative flex-1 max-w-sm">
        ...
        <input 
          ...
          class="w-full pl-10 pr-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all placeholder:text-[#52525b]"
        />
      </div>
      <div class="flex items-center bg-white/5 border border-white/10 rounded-lg px-1">
        {#each symbols as symbol}
          <button 
            onclick={() => symbolFilter = symbol}
            class="px-3 py-1.5 text-xs font-medium rounded-md transition-all {symbolFilter === symbol ? 'bg-white/10 text-white shadow-sm' : 'text-[#71717a] hover:text-[#a1a1aa]'}"
          >
            {symbol}
          </button>
        {/each}
    ...
    <div class="flex items-center gap-2">
      <button class="flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-xs font-medium text-[#a1a1aa] hover:text-white hover:bg-white/10 transition-all">
        <Calendar size={14} />
        Last 30 Days
      </button>
  ...
  <div class="rounded-xl border border-white/5 bg-[#0c0c0e] overflow-hidden">
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-white/5">
          {#each filteredTrades as trade}
            <tr class="text-sm hover:bg-white/[0.02] transition-colors group cursor-default">
              <td class="px-6 py-4">
                <div class="flex flex-col">
                  <span class="text-[#fafafa] font-medium">{new Date(trade.entry_time).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
              ...
              <td class="px-6 py-4 text-[#a1a1aa] font-mono text-xs capitalize">{trade.pattern.replace('is_', '').replace(/_/g, ' ')}</td>
              <td class="px-6 py-4">
                <div class="flex flex-col">
                  <span class="text-[#a1a1aa]">₹{trade.entry_price.toFixed(2)}</span>
    ...
    {#if filteredTrades.length === 0}
      <div class="p-20 flex flex-col items-center justify-center text-center">
        <div class="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center mb-4">
          <Search size={20} class="text-[#3f3f46]" />
        </div>
        <h4 class="text-sm font-medium text-[#a1a1aa]">No trades found</h4>
    ...
    <div class="p-4 border-t border-white/5 flex items-center justify-between">
      ...
      <div class="flex items-center gap-1">
        <button disabled class="px-3 py-1.5 text-xs font-medium text-[#3f3f46] cursor-not-allowed">Previous</button>
        <button class="px-3 py-1.5 text-xs font-medium bg-white/5 text-white rounded-md border border-white/10 hover:bg-white/10 transition-all">Next</button>
      </div>
    </div>
  </div>
</div>
