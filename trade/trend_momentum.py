...
class TrendMomentumStrategy:
    ...
    def reset_state(self):
        """Reset internal state for backtest or live trading."""
        ...
        if not hasattr(self, 'manual_exits'):
            self.manual_exits = set() # Store entry_times of manually exited trades
    ...
    def force_exit(self, entry_time: str, exit_price: float, exit_time: str):
        ...
        if self.active_trade and self.active_trade.entry_time == entry_time:
            self.active_trade.exit_time = exit_time
    ...
    def run_backtest(self, df_lower: pd.DataFrame, df_upper: pd.DataFrame) -> List[Trade]:
        ...
        for i in range(50, len(df_lower)):
            ...
            if self.active_trade:
                # Check if this trade was manually exited
                if self.active_trade.entry_time in self.manual_exits:
                    # Logic should have already handled this in the previous loop iteration 
                    # but if we are re-running backtest, we need to respect it.
                    # This is tricky because we need the exit price.
                ...
                if self.active_trade.option_type == 'CALL':
                    ...
                    if self.trailing_enabled:
                        ...
                        if profit_r >= activation_r:
                            ...
                            atr_trail = self.highest_price_since_entry - (row['atr'] * trail_multiplier)
                            if new_sl is None:
                                new_sl = atr_trail
                            else:
                    ...
                    if self.trailing_enabled:
                        ...
                        if profit_r >= activation_r:
                            trail_multiplier = self.trailing_config.get('multiplier', 1.2)
                            atr_trail = self.lowest_price_since_entry + (row['atr'] * trail_multiplier)
                            if new_sl is None:
                                new_sl = atr_trail
                            else:
